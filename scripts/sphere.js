...
Photosphere.prototype.loadPhotosphere = function(holder){
	holder.innerHTML = "<div class='preloader-wrapper big active load-sphere'><div class='spinner-layer spinner-orange-only'><div class='circle-clipper left'><div class='circle'></div></div><div class='gap-patch'><div class='circle'></div></div><div class='circle-clipper right'><div class='circle'></div></div></div></div>";
...
Photosphere.prototype.start3D = function(image){
	...
	this.camera = new THREE.PerspectiveCamera( 65, parseInt(this.holder.offsetWidth) / parseInt(this.holder.offsetHeight), 1, 1100 );
	this.scene = new THREE.Scene();
	// Sphere Geometry: http://threejs.org/docs/#Reference/Extras.Geometries/SphereGeometry (size, rows, columns)
	mesh = new THREE.Mesh( new THREE.SphereGeometry( 256, 64, 64 ), this.loadTexture( image ) );
	mesh.scale.x = - 1;
	this.scene.add( mesh );
